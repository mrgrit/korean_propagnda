# K-Propaganda — P0 엔진
A 후보자의 지지율을 n% 올리려면 어떤 매체에 어떤 메시지를 줘야 할까? / 어떤 정책적 발언 또는 네거티브가 좋을까 / 어디가서 어떤 활동을 하는게 좋을까 등등을 생성해준다.

- 한국인 100만명 페르소나데이터를 중심으로 온갖 오픈 데이터셋을 다 긁어서 실제 처럼 움직이게 했다.
- 7개 max 구독 요금제 에이전트 토큰을 한계치 까지 써서 메시지에 대한 반응을 시뮬레이션 한다.
- '현대 사회학(엔써니 기든스)' 교재에 나오는 모든 현상과 현대 발달된 기술 환경을 반영하여 진짜 사회가 돌아가게 했다.
...
- **조작자 LLM 전략가** (`manipulator.mode: llm`): 라운드마다 90개 인구 집단(권역 5 × 세대 6 × 학력 3)의 집계(규모·설득가능성·리터러시·이동가능비율·최근 수율·선호 채널·지난 노출)와 자기 라운드 기록(수율·방어 탐지·되돌림·ΔA)을 브리핑으로 받고, **프레임·주장 유형(윤리 상한 이하)·표적 집단 가중치·채널 비중·예산 강도**를 JSON으로만 결정한다. 메시지 문구는 쓰지 않으며(템플릿 유지), 개인 데이터는 보지 않는다. 답이 무효면 규칙 기반으로 대체하고 `manipulator.llm.used=false`로 기록.
- **방어자 LLM 전략가** (`defender.mode: llm`): 탐지는 규칙 그대로, 탐지 후 정정 예산을 어느 집단·채널에, **사후 정정 vs 사전 예방접종** 비율로 쓸지 결정. 예방접종 예산은 아직 노출되지 않은 취약 집단에 면역(저항)만 올린다.
- **L2 확대**: `backend.batch_size`(기본 8) 페르소나를 한 호출에 묶어 배열로 응답받는다. 구독 세션 호출 수가 1/8로 줄어 K를 크게 잡을 수 있다. 전략가는 `backend.strategy_model`(기본 sonnet, 라운드당 2회).
- **반응 톤**: `configs/tone_bank.yaml`의 세대·권역별 말투 힌트를 페르소나 카드에 주입. 현재는 휴리스틱 자리표시자이며 라이선스 확인 후 한국어 코퍼스(BIGKinds 등)에서 추출한 서술로 교체.
- **누적 학습(전략 메모리)**: LLM 모드 실행이 끝나면 `data/strategy_memory.json`에 실행별 집계(프레임·주장유형·집단·주채널별 노출/이동 수율, 방어 탐지율·되돌림률, 예방접종 비율별 효과)가 쌓이고, 다음 실행의 브리핑에 "누적 경험" 표로 들어간다(자기 실행은 제외). 개인 데이터는 저장하지 않는다. 웹 UI "전략 메모리" 패널에서 보고 비울 수 있다. `manipulator.use_memory: false`로 끌 수 있다.
- **L2 사회적 맥락**: 페르소나 카드에 주변 지인 성향(관계망 이웃 lean), 반복 노출 여부가 들어가고, 2차(전파) 승격자는 실제 공유자가 남긴 반응 문장을 "지인이 공유하며 한 말"로 함께 받는다.
- **톤 뱅크 빌더**: `opinion-field build-tone-bank` 또는 웹 UI "톤 뱅크 생성" — Nemotron 페르소나 문장(CC-BY-4.0 한국어 코퍼스)에서 세대 6 × 권역 5 = 30개 집단의 말투·관심 서술을 LLM으로 요약해 `configs/tone_bank.yaml`(`by_group`)을 만든다. 외부 코퍼스는 라이선스 확인 후 같은 형식으로 교체.
- 전략가 결정·근거는 `rounds.jsonl`의 `manipulator.llm` / `defense.llm`과 리포트 "전략가 결정" 절에 남고 웹 UI 조작자 계획 상자와 조작 vs 방어 로그에 표시된다. 체크포인트에 전략가 기록이 포함돼 재개 시 학습 맥락이 유지된다.

## P2+ (계획)

11채널, 목표 다종화, 조건 스윕, 방어 타이밍 게임, UI 완전 연동.
